...
rg -Fq '\[\[󰌠' "$starship_config"
rg -Fq '\[[v$output]' "$starship_config"
rg -Fq "symbol = ' '" "$starship_config"
rg -Fq "symbol = ' '" "$starship_config"
rg -Fq "symbol = ' '" "$starship_config"
rg -Fq "symbol = ' '" "$starship_config"
test "$(rg -Fc '\[[$symbol$version]' "$starship_config")" -eq 4
rg -Fq '\[[ v$output]' "$starship_config"
rg -Fq "detect_env_vars = ['VIRTUAL_ENV', '!CONDA_DEFAULT_ENV']" "$starship_config"
rg -Fq "format = 'took [\$duration](\$style) '" "$starship_config"
! rg -Fq "format = 'via " "$starship_config"
